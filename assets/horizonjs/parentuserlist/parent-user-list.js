...
$(document).ready(function () {
    ...
    function populateParentUserList() {
        $.ajax({
            type: 'GET',
            contentType: 'application/json',
            url: 'https://evd-server.herokuapp.com/api/users/get_user',
            headers: {
                'X-Auth-Token': savedToken
            },
            success: function (data) {
    ...
    $('#html5-extension tbody').on( 'click', '#user_id', function () {
        ...
        // e.preventDefault();
        // window.open(this.href+"?user_id="+data._id, );
        // window.location.href="fundTransfer.html?user_id="+data._id
        // alert( data._id +"'s salary is: "+ data );
        localStorage.setItem("user_id",data._id)
    } );

    checkIfUserAlreadyLoggedIn();
    if (!userLoggedIn) {
        populateParentUserList()
    }
});
// endregion
